...
function BackgroundManager({ signageId }) {
  ...
  const loadBackground = async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/signage/${signageId}/background`);
      if (res.ok) {
      ...
      // Load logo URL
      const signageRes = await apiFetch(`/api/signage/${signageId}`);
      if (signageRes.ok) {
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Signage display background and logo</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
          ...
          {preview && (
            <div className="relative">
              ...
              <p className="text-xs text-gray-500 mt-2 text-center">
                This is how the background will appear on the signage display
              </p>
              {backgroundConfig.type === 'image' && backgroundConfig.url && (
